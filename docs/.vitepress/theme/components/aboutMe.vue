<template>
  <div class="about-me-container">
    <div class="title">😀关于笔者</div>
    <div class="content">

      <div class="card-container">
        <div class="e-card playing">
          <div class="image"></div>

          <div class="wave"></div>
          <div class="wave"></div>
          <div class="wave"></div>

          <div class="infotop">
            <div class="img-container">
              <img src="https://oss.justin3go.com/justin3goAvatar.png" alt="" style="height: 80px">
            </div>
            <br>Justin3go<br>
            <div class="name">前端工程师 & 计算机爱好者</div>
            <div class="name">喜欢折腾一些效率工具，尝试各种工具软件和工具网站，热爱开源、分享、探索、用代码创造价值</div>
            <div class="social-link">
              <div class="card">
                <t-popup class="placement top center" content="github" placement="top" show-arrow destroy-on-close>
                  <a class="social-link1" href="https://github.com/Justin3go/justin3go.github.iomai">
                    <svg viewBox="0 0 496 512" height="1em" fill="#fff" xmlns="http://www.w3.org/2000/svg">
                      <path
                        d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z">
                      </path>
                    </svg>
                  </a>
                </t-popup>
                <t-popup class="placement top center" content="twitter/x" placement="top" show-arrow destroy-on-close>
                  <a class="social-link2" href="https://twitter.com/_Justin3go">
                    <svg viewBox="0 0 16 16" class="bi bi-twitter" fill="currentColor" height="16" width="16"
                      xmlns="http://www.w3.org/2000/svg" style="color: white">
                      <path fill="white"
                        d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z">
                      </path>
                    </svg> </a>
                </t-popup>
                  <t-popup class="placement top center" content="发送邮件" placement="top" show-arrow destroy-on-close>
                    <a class="social-link3" href="mailto:just@justin3go.com?subject=请表明您的来意">
                      <svg t="1699149704102" class="bi bi-whatsapp" fill="currentColor" viewBox="0 0 1024 1024" version="1.1"
                        xmlns="http://www.w3.org/2000/svg" p-id="10969" width="16" height="16">
                        <path
                          d="M153.236734 163.111422l714.763665 0c48.810633 0 88.718697 38.475467 89.332667 85.853503l-446.458679 242.518237L64.313381 249.169581C64.722694 201.689218 104.323773 163.111422 153.236734 163.111422L153.236734 163.111422zM64.313381 341.981413l-0.409313 429.062856c0 47.787349 40.215049 86.876786 89.332667 86.876786l714.763665 0c49.117618 0 89.332667-39.089437 89.332667-86.876786L957.333067 341.776756 521.311882 573.038873c-6.651344 3.581493-14.735285 3.581493-21.386629 0L64.313381 341.981413 64.313381 341.981413zM64.313381 341.981413"
                          p-id="10970" fill="#ffffff"></path>
                      </svg>
                    </a>
                  </t-popup>
                <t-popup class="placement top center" placement="top" show-arrow destroy-on-close>
                  <template #content>
                    <div style="height: 160px; width: 117.2px;">
                      <img src="https://oss.justin3go.com/weixin.jpg" alt="" style="height: 160px;">
                    </div>
                  </template>
                  <a class="social-link4">
                    <svg t="1699181349640" class="bi bi-whatsapp" fill="currentColor" viewBox="0 0 1280 1024"
                      version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="14538" width="16" height="16">
                      <path
                        d="M858.148572 309.753124C872.728381 309.753124 887.070474 310.852936 901.491809 312.42409 862.665144 133.155348 669.403428 0 448.725335 0 202.057143 0 0 166.620943 0 378.334024 0 500.491295 67.193905 600.809513 179.553524 678.73878L134.704762 812.522593 291.596191 734.593326C347.696763 745.512852 392.704 756.825165 448.725335 756.825165 462.829716 756.825165 476.775619 756.1967 490.563046 755.17545 481.846856 725.402071 476.696381 694.293212 476.696381 661.848868 476.775619 467.497047 645.077335 309.753124 858.148572 309.753124L858.148572 309.753124ZM616.947809 189.167012C650.861716 189.167012 673.127619 211.241733 673.127619 244.707328 673.127619 278.015805 650.861716 300.3262 616.947809 300.3262 583.430093 300.3262 549.674665 278.015805 549.674665 244.707328 549.674665 211.163177 583.350856 189.167012 616.947809 189.167012L616.947809 189.167012ZM302.927237 300.3262C269.251046 300.3262 235.337143 278.015805 235.337143 244.707328 235.337143 211.241733 269.251046 189.167012 302.927237 189.167012 336.603428 189.167012 358.948572 211.163177 358.948572 244.707328 358.948572 278.015805 336.603428 300.3262 302.927237 300.3262L302.927237 300.3262ZM1267.809526 656.506941C1267.809526 478.652242 1088.256 333.713234 886.595046 333.713234 673.048381 333.713234 504.984381 478.730803 504.984381 656.506941 504.984381 834.832988 673.127619 979.300652 886.595046 979.300652 931.285335 979.300652 976.371809 968.224012 1021.220572 957.068815L1144.277335 1024 1110.521907 912.762255C1200.615619 845.673953 1267.809526 756.825165 1267.809526 656.506941L1267.809526 656.506941ZM762.825144 600.809513C740.559237 600.809513 717.976381 578.813348 717.976381 556.345841 717.976381 534.192558 740.559237 511.882163 762.825144 511.882163 796.897526 511.882163 819.004954 534.192558 819.004954 556.345841 819.004954 578.813348 796.897526 600.809513 762.825144 600.809513L762.825144 600.809513ZM1009.651809 600.809513C987.544381 600.809513 965.040763 578.813348 965.040763 556.345841 965.040763 534.192558 987.465144 511.882163 1009.651809 511.882163 1043.407237 511.882163 1065.831619 534.192558 1065.831619 556.345841 1065.831619 578.813348 1043.407237 600.809513 1009.651809 600.809513L1009.651809 600.809513Z"
                        fill="#ffffff" p-id="14539"></path>
                    </svg>
                  </a>
                </t-popup>
              </div>
            </div>
          </div>
        </div>
      </div>
      <figure class="lang-chart-container">
        <div class="chart-title">技术语言偏好</div>
        <iframe class="lang-chart" :src="langChartSrc"></iframe>
      </figure>
    </div>
  </div>
</template>
<script setup lang="ts">
import { useData } from "vitepress";
import { computed } from "vue";

const { isDark } = useData();
const langChartSrc = computed(() =>
  isDark.value
    ? "https://wakatime.com/share/@a210bf50-f97f-4461-adae-6661a6273a34/41e204ec-87e7-4ee5-850f-51ca6a3ac2ae.svg"
    : "https://wakatime.com/share/@a210bf50-f97f-4461-adae-6661a6273a34/093e0115-66ad-4945-be11-e39ba69b7f82.svg")

const bgColor = computed(() => isDark.value ? "#222F38" : "#fff")
</script>
<style lang="scss" scoped>
.about-me-container {
  width: 100%;
  height: 100%;
  padding: 20px 40px;

  iframe {
    border: none;
  }

  .title {
    font-size: 20px;
    font-weight: 900;
  }

  .content {
    display: flex;
    justify-content: left;
    width: 100%;
    height: 100%;
    margin-top: 20px;

    .lang-chart-container {
      flex: 1;
      margin: 10px;
      padding: 20px;
      padding-top: 0;
      border-radius: 16px;
      background-color: v-bind(bgColor);
      box-shadow: 0px 8px 28px -9px rgba(0, 0, 0, 0.45);

      display: flex;
      justify-content: space-around;

      .chart-title {
        margin-top: 20px;
        writing-mode: vertical-rl;
        letter-spacing: 10px;
        font-size: 20px;
        font-weight: 900;
        background: linear-gradient(744deg, #af40ff, #5b42f3 60%, #00ddeb);
        background-clip: text;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
      }

      .lang-chart {
        width: 600px;
        height: 440px;
      }
    }

  }

  .card-container {
    padding: 10px;

    .e-card {
      background: transparent;
      box-shadow: 0px 8px 28px -9px rgba(0, 0, 0, 0.45);
      position: relative;
      width: 340px;
      height: 480px;
      border-radius: 16px;
      overflow: hidden;
    }

    .wave {
      position: absolute;
      width: 540px;
      height: 700px;
      opacity: 0.6;
      left: 0;
      top: 0;
      margin-left: -20%;
      margin-top: -20%;
      background: linear-gradient(744deg, #af40ff, #5b42f3 60%, #00ddeb);
    }

    .icon {
      width: 3em;
      margin-top: -1em;
      padding-bottom: 1em;
    }

    .infotop {
      text-align: center;
      font-size: 20px;
      position: absolute;
      top: 5.6em;
      left: 0;
      right: 0;
      color: rgb(255, 255, 255);
      font-weight: 600;

      .img-container {
        display: flex;
        justify-content: center;
      }
    }

    .name {
      padding: 5px 40px;
      font-size: 14px;
      font-weight: 100;
      position: relative;
      top: 1em;
      text-transform: lowercase;
    }

    .wave:nth-child(2),
    .wave:nth-child(3) {
      top: 210px;
    }

    .playing .wave {
      border-radius: 40%;
      animation: wave 3000ms infinite linear;
    }

    .wave {
      border-radius: 40%;
      animation: wave 55s infinite linear;
    }

    .playing .wave:nth-child(2) {
      animation-duration: 4000ms;
    }

    .wave:nth-child(2) {
      animation-duration: 50s;
    }

    .playing .wave:nth-child(3) {
      animation-duration: 5000ms;
    }

    .wave:nth-child(3) {
      animation-duration: 45s;
    }

    @keyframes wave {
      0% {
        transform: rotate(0deg);
      }

      100% {
        transform: rotate(360deg);
      }
    }

    .social-link {
      margin-top: 20px;
      display: flex;
      justify-content: center;

      .card {
        display: flex;
        height: 55px;
        width: 220px;

        svg {
          position: absolute;
          display: flex;
          width: 60%;
          height: 60%;
          font-size: 24px;
          font-weight: 700;
          opacity: 1;
          transition: opacity 0.25s;
          z-index: 2;
          cursor: pointer;
        }

        .social-link1,
        .social-link2,
        .social-link3,
        .social-link4 {
          position: relative;
          display: flex;
          align-items: center;
          justify-content: center;
          width: 25%;
          color: whitesmoke;
          font-size: 24px;
          text-decoration: none;
          transition: 0.25s;
          border-radius: 50px;
        }

        svg {
          transform: scale(.6);
        }

        .social-link1:hover {
          background-color: #242c34;
          animation: bounce_613 0.4s linear;
        }

        .social-link2:hover {
          background-color: #00ccff;
          animation: bounce_613 0.4s linear;
        }

        .social-link3:hover {
          background-color: #5865f2;
          animation: bounce_613 0.4s linear;
        }

        .social-link4:hover {
          background-color: #12a50b;
          animation: bounce_613 0.4s linear;
        }

        @keyframes bounce_613 {
          40% {
            transform: scale(1.4);
          }

          60% {
            transform: scale(0.8);
          }

          80% {
            transform: scale(1.2);
          }

          100% {
            transform: scale(1);
          }
        }
      }
    }
  }
}

@media (max-width: 1100px) {
  .about-me-container {
    padding: 20px 10px;

    .lang-chart-container {
      display: none !important;
    }

    .card-container {
      width: 100%;

      .e-card {
        width: 100%;
      }

      .wave {
        width: 200%;
        height: 250%;
      }
    }
  }
}
</style>
